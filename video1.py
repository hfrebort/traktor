#!/usr/bin/python3
import cv2, time, render, detector
import numpy as np
...
while(True):
    ...
    if frame is not None:
        ...
        #default_img, offset = detector.detect(img, contourMode='MARK')
        #default_img = render.render(default_img, offset)

        default_img = draw_on_frame(img, default_img)
        cv2.imshow('default', default_img)
        print(f"Prepare image took: {int((time.perf_counter() - start)*1000)} ms - offset: {offset}")
    else:
        cap.release()
        cap = cv2.VideoCapture(video_name)
